...
请开启 Num Lock，并避免将 Shift 与小键盘数字组合使用。快捷键仅在《绝地潜兵 2》处于前台时生效。修饰键为空时不提供按住预览，启用浮窗时仍会显示执行状态。修改快捷键后需重启程序。

选择顺序和自动准备也可以在托盘菜单中切换。

托盘菜单中的 **Overlay monitor** 可选择跟随游戏或固定显示器。
