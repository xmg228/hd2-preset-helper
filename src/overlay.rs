...
use crate::app_events::{
    APP_EVENT_MESSAGE, AppEvent, AppEventSink, OverlayPreset, OverlayPresetStatus,
};
    ...
    OVERLAY.with(|overlay| {
        ...
        match event {
            AppEvent::PresetListUpdated { presets } => {
                ...
                state.metrics = state.metrics.with_preset_count(state.presets.len());
                warm_preset_icons(state);
                state.set_ready();
                Some(OverlayEventPolicy::HideAfter(READY_HIDE_DELAY))
            }
            ...
            AppEvent::PresetSaved {
                ...
                if let Some(row) = state.presets.iter_mut().find(|row| row.name == preset) {
            ...
            AppEvent::UiStateDetected { state: ui_state } => {
                state.status = format!("Detected UI: {ui_state}");
                state.accent = rgb(255, 220, 96);
                Some(OverlayEventPolicy::Hold)
            }
            AppEvent::ListSelectionStarted {
                ...
                state.selected_count = 0;
                state.requested_count = requested_items;
                state.status = format!("Selecting {}: 0/{requested_items}", item_kind.label());
                state.accent = rgb(255, 220, 96);
                Some(OverlayEventPolicy::Hold)
            }
            AppEvent::ItemSelected { item_id } => {
                state.selected_count += 1;
                let progress = if state.requested_count > 0 {
                    format!("{}/{}", state.selected_count, state.requested_count)
                } else {
                    state.selected_count.to_string()
                };
                state.status = format!(
                    "Selected {progress}: {}",
                    display_item_name(&state.catalog, &item_id)
                );
                state.accent = rgb(255, 220, 96);
                Some(OverlayEventPolicy::Hold)
            }
            ...
            AppEvent::PresetFailed { preset, error } => {
                state.active_preset = Some(preset);
                state.status = format!("Failed: {}", first_error_line(&error));
                state.accent = rgb(255, 120, 120);
                Some(OverlayEventPolicy::HideAfter(FAILED_HIDE_DELAY))
            }
        }
    })
...
fn paint_overlay(hwnd: HWND) {
    OVERLAY.with(|overlay| {
        ...
        unsafe {
            ...
            renderer.fill_rect(hdc, &accent_rect, state.accent);
...
fn draw_presets(hdc: HDC, state: &OverlayState, renderer: &OverlayRenderer) {
    unsafe {
        let visible_count = state.presets.len().min(MAX_OVERLAY_ROWS);
        for (index, preset) in state.presets.iter().take(visible_count).enumerate() {
            let metrics = state.metrics;
            let y = metrics.row_y + index as i32 * (metrics.row_h + metrics.row_gap);
            let is_active = state.active_preset.as_deref() == Some(preset.name.as_str());
...
fn warm_preset_icons(state: &mut OverlayState) {
    ...
    for item_id in item_ids {
        ...
        let icon = match render_item_icon(&state.catalog, &item_id, state.metrics.icon_size) {
            Ok(icon) => Some(icon),
            Err(error) => {
...
fn display_item_name<'a>(catalog: &'a IconCatalog, item_id: &'a str) -> &'a str {
    catalog
        .get(item_id)
