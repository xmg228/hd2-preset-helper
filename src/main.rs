...
impl Default for PresetsConfig {
    fn default() -> Self {
        Self {
            path: PathBuf::from("data/presets.json"),
            auto_ready_up: false,
        }
    }
}
...
fn run_preset_hotkey_mode(config: AppConfig, config_path: &Path) -> Result<()> {
    ...
    let registered_hotkeys = input::RegisteredHotkeys::register(&hotkeys)?;
    let action_config = PresetActionConfig {
        presets: &config.presets.path,
