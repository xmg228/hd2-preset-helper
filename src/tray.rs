...
pub const EXIT_MESSAGE: u32 = WM_APP + 1;
...
pub fn spawn() -> Result<TrayHandle> {
    ...
    let thread = std::thread::Builder::new()
        ...
        .spawn(move || {
            ...
            let result = run_tray(owner_thread_id, tray_thread_id, &ready_tx);
...
fn run_tray(
    ...
    MenuEvent::set_event_handler(Some(move |event: MenuEvent| {
        if event.id == close_id {
            let _ =
                unsafe { PostThreadMessageW(owner_thread_id, EXIT_MESSAGE, WPARAM(0), LPARAM(0)) };
